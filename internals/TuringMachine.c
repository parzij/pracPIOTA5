...
static void tm_cell_expand_left(TM_Machine *tm)
{
...
static void tm_cell_expand_right(TM_Machine *tm)
{
...
static bool add_transition(TM_State *st, TM_Transition tr)
{
    bool ok = false;
...
bool tm_init(TM_Machine *tm, const char *fname, char blank)
{
    ...
    if (ok) {
        while (1) {
            ...
            if (res == EOF) { ok = true; break; }
            if (res != 5 || st >= TM_MAX_STATE || next >= TM_MAX_STATE) { ok = false; break; }

            TM_Transition tr;
            tr.read = read; tr.write = write; tr.move = dir; tr.next_state = next;
            if (!add_transition(&tm->states[st], tr)) { ok = false; break; }

            if (st + 1 > tm->state_qty)  { tm->state_qty  = st + 1; }
            if (next + 1 > tm->state_qty){ tm->state_qty  = next + 1; }
        }
        fclose(fp);
    }
    return ok;
...
void tm_load_tape(TM_Machine *tm, const char *input)
{
    ...
    left->sym = tm->blank; left->left = NULL; left->right = NULL;
    tm->leftmost = left;

    TM_Cell *cur = left;

    /* добавляем символы строки */
    ...
    right->sym = tm->blank; right->right = NULL; right->left = cur;
    cur->right = right;

    tm->rightmost = right;
    tm->head      = left->right; /* головка на первый введённый символ */
    tm->cur_state = 1;           /* q1 */
    tm->step_counter = 0;
}

static TM_Transition *find_transition(TM_Machine *tm, char sym)
{
...
bool tm_step(TM_Machine *tm)
{
    ...
    if (tr == NULL) {
        /* неопределённость — останавливаем машину */
        tm->cur_state = 0;
    } else {
        /* выполняем тройку действий */
        tm->head->sym = tr->write;

        if (tr->move == 'L') {
            if (tm->head->left == NULL) { tm_cell_expand_left(tm); }
            tm->head = tm->head->left;
        } else if (tr->move == 'R') {
            if (tm->head->right == NULL) { tm_cell_expand_right(tm); }
    ...
    tm->step_counter += 1;
    if (tm->cur_state == 0) { halted = true; }
    return halted;
}

/* печать не более ±30 ячеек вокруг головки для компактности */
void tm_print(const TM_Machine *tm)
{
    ...
    while (cur != NULL && pos < 2 * window) {
        putchar(cur->sym);
        if (cur == tm->head) { head_pos = pos; }
        cur = cur->right;
        pos++;
    }
    putchar('\n');

    /* вторая строка – указатель головки */
    int i = 0;
    while (i < head_pos) { putchar(' '); i++; }
    putchar('^');
    putchar('\n');
